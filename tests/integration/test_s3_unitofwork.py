...
class TestS3DecompUnitOfWork:
    ...
    @pytest.mark.asyncio
    async def test_download_and_extract(self, s3_with_decomp_case, tmp_path, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_on_exception(self, s3_with_decomp_case, tmp_path, monkeypatch):
...
class TestS3NewaveUnitOfWork:
    ...
    @pytest.mark.asyncio
    async def test_download_and_extract(self, s3_with_newave_case, tmp_path, monkeypatch):
        ...
        try:
            async with S3NewaveUnitOfWork(
                s3_repo, "newave-bucket", "newave123"
            ) as uow:
                # Verify temp dir created
                assert uow.temp_dir is not None
                assert uow.temp_dir.exists()
...
class TestS3DecompProspectionUnitOfWork:
    ...
    @pytest.mark.asyncio
    async def test_download_multiple_sources(self, s3_with_multiple_cases, tmp_path, monkeypatch):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_all_temp_dirs(self, s3_with_multiple_cases, tmp_path, monkeypatch):
